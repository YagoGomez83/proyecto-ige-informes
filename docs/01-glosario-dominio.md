...
| **Causa** | Expediente judicial/policial de la **Dependencia solicitante** (carátula + N° de pieza sumarial, ambos obligatorios; + circunscripción judicial, opcional). Pertenece al Informe, no al Caso — se completa cuando una dependencia formaliza su pedido de documentación. |
| **Circunscripción judicial** | Una de tres divisiones judiciales de la provincia: **Primera**, **Segunda** o **Tercera**. Campo opcional de la Causa — algunos expedientes no la especifican. |
...
| **Tipo de Causa** | Catálogo preseteado de carátulas reutilizables (ej. `AV. ROBO`, `AV. HURTO CALIFICADO`, `TAREA INVESTIGATIVA`) — se elige al completar la Carátula de una Causa en vez de tipearla como texto libre. No reemplaza a Causa: el N° de pieza sumarial y la circunscripción judicial siguen siendo específicos de cada expediente. Si el Tipo de Causa real no está en el catálogo, se agrega primero (Admin) — mismo criterio que Dependencia. |
...
| **Vehículo** | Rodado en catálogo de vigilancia. Atributos: marca/modelo, color, dominio (puede ser parcial/incierto), **tipo** (Auto | Moto | Camioneta | Camión), **categoría de alerta** (Robado, Narcotráfico, Inhibidores, Robo de Cubiertas, Pedido Especial — puede tener más de una), **estado** (Vigente | Identificado), **acción a realizar** al detectarlo (Detener | Identificar | Sin acción), **avisar a** (dependencia/persona de contacto), **fecha de baja** (cuándo dejó de estar en vigilancia activa). "Sin acción" es para un Vehículo cargado solo como referencia (ya identificado/vinculado a un Caso/Informe/Análisis pasado, sin pedido activo) — no implica ninguna intervención si se lo vuelve a detectar, pero queda en el catálogo para consultar su historial. `Marca` y `Color` se eligen de un `<select>` alimentado por **MarcaVehiculo**/**ColorVehiculo** (con opción "Otra" para texto libre) — pero siguen siendo `string` en `Vehiculo`, no una FK. `Tipo` es un enum fijo (no un catálogo administrable). Si `Tipo = Moto`, `Cilindrada` (texto libre, ej. `110cc`) es **obligatoria**; para cualquier otro tipo queda `null` — validado como invariante en el constructor del dominio. Puede tener 0 o más **VehiculoImagen** asociadas. |
| **MarcaVehiculo** | Catálogo preseteado de marcas de vehículo (Ford, Fiat, Volkswagen, etc. — las más comunes en Argentina) para elegir en el `<select>` de Marca al registrar un Vehículo, en vez de tipearla siempre a mano. No es una FK desde `Vehiculo` — solo alimenta el valor de texto. Si la marca real no está, se puede tipear libremente ("Otra") o agregarla primero al catálogo (Admin). |
| **ColorVehiculo** | Catálogo preseteado de colores de vehículo (Blanco, Negro, Gris, etc.), mismo criterio que **MarcaVehiculo** pero para el campo Color. |
| **VehiculoImagen** | Cada fotografía adjunta a un Vehículo del catálogo: archivo de imagen, fecha de carga y quién la subió. Se administra desde la ficha del propio Vehículo, no desde un Informe — distinta de **Evidencia**, que es la captura documentada dentro de un Informe. |
| **Persona** | Individuo mencionado o identificado en un Caso/Informe, con un **rol** (Denunciante, Damnificado, Sospechoso, Conductor identificado, Testigo). Puede tener DNI si está identificada, o solo características físicas si no. Puede tener 0 o más **PersonaImagen** asociadas. |
| **PersonaImagen** | Cada fotografía adjunta a una Persona del catálogo: archivo de imagen, fecha de carga y quién la subió. Se administra desde la ficha de la propia Persona, no desde un Informe — mismo criterio que **VehiculoImagen**. |
| **PersonaVehiculo** | Vínculo directo entre una Persona y un Vehículo (ej. Persona identificada como conductora de ese Vehículo), independiente de los Informes/Casos en los que ambos aparezcan. Visible desde ambas fichas. |
| **Alerta** | Aviso automático generado cuando se vincula un Vehículo o Persona a un Informe (ver **Evidencia**) y el sistema detecta que ese Vehículo/Persona **ya estaba vinculado a otro Informe anterior** (Reincidencia) o **existía en el catálogo sin ningún Informe vinculado todavía** (Carga huérfana). Visible para todo Analista/Supervisor/Admin en `/alertas`, sin destinatario individual — cualquiera de esos roles puede marcarla atendida. Distinta de **categoría de alerta** de Vehículo (que es una etiqueta de catálogo, no una notificación). |
...
| **Borrado lógico** | Marcar un Informe, Caso de Análisis, Vehículo o Persona como `Eliminado` (con `FechaEliminacion`) en vez de borrar la fila de la base — el registro deja de aparecer en listados/búsquedas/analítica pero se conserva para auditoría. Exclusivo de Supervisor/Admin. Distinto de "Dar de baja" (Vehículo, solo fin de vigilancia activa, sigue visible) y de "Cerrar" (Caso, solo cambia el Estado). Un Informe Publicado no admite borrado lógico (inmutable). |
...
| **Migración Pendiente** | PDF histórico subido en una migración masiva (HU-04) cuyo parser no pudo reconocer el ID Registro y/o la Fecha de Análisis — no llega a crear un Informe. Se guarda el PDF original en MinIO junto con los demás datos ya extraídos (Causa, Relato, etc., y el ID Registro si sí se reconoció) para que el Administrador complete el/los dato/s faltante/s manualmente después, desde `/informes/migrar/pendientes`, sin tener que volver a subir el archivo. Al completarla, se crea el Informe real (`Informe.CrearMigrado`) y la Migración Pendiente se elimina. Entidad transitoria — no es un estado del Informe (que siempre exige ID Registro y Fecha de Análisis), es un paso previo a que el Informe exista. |
