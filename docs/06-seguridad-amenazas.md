...
| **R**epudiation — negar haber modificado/consultado algo | Medio | `AuditLog` append-only, sin permisos de borrado ni para Admin desde la aplicación. Incluye el botón "Reintentar" de `/informes/migrar/pendientes` (`ReprocesarMigracionPendienteQueryHandler`, HU-04): aunque no persiste nada, vuelve a leer el PDF de una `MigracionPendiente` (dato de investigación) — audita `"ReprocesarPdf"` sobre `MigracionPendiente` en toda lectura, y `"ExtraccionPdf"` sobre `Persona` cuando el parser encuentra DNI, mismo criterio que `ParsearPdfInformeQueryHandler` (hallazgo del security-reviewer: faltaba `IAuditLogger` por completo en la primera versión) |
