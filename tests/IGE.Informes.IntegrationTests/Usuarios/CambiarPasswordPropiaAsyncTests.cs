...
public class CambiarPasswordPropiaAsyncTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task CambiarPasswordPropia_CaminoFeliz_PermiteLoginConLaNuevaYNoConLaViejaEInvalidaElStamp()
    {
        ...
        await using (var stack = new IdentityStack(options))
        {
            ...
            var exito = await service.CambiarPasswordPropiaAsync(
                usuarioId, "ContraseñaVieja123", "ContraseñaNueva456", CancellationToken.None);
    ...
    [Fact]
    public async Task CambiarPasswordPropia_ConContraseñaActualIncorrecta_DevuelveFalseYNoInvalidaElStamp()
    {
        var options = new DbContextOptionsBuilder<AppDbContext>()
            .UseNpgsql(_postgres.GetConnectionString())
            .Options;
        ...
        await using (var stack = new IdentityStack(options))
        {
            ...
            var service = new UserManagementService(stack.UserManager, stack.DbContext);
            exito = await service.CambiarPasswordPropiaAsync(
                usuarioId, "ContraseñaIncorrecta", "ContraseñaNueva456", CancellationToken.None);
        }
